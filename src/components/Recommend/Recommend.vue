...
//        console.log("recommend.vue",this.$store.state.openid);
      ...
      login() {
        if (this.$store.state.openid == 0 || this.$store.state.openid == undefined || this.$store.state.openid == -1 || this.$store.state.userID != 0) {
          return
        }
      ...
      checkUserMsg() {
        ...
        this.$http.get(check_url, {headers: {Authorization: "Basic " + btoa("screen:screen123")}}).then(res => {
          ...
          var headImgMsmp = data.substring(data.indexOf("<avatar>") + 8, (data.indexOf("</avatar>")));   //储存用户头像
          this.$store.commit("headImgMsmp", headImgMsmp);

          if (nickNameMsmp == "") {
            var nickName = this.$store.state.nickName;
          } else {
            var nickName = nickNameMsmp;
          }

          var userinfo_params = "userid=" + this.$store.state.userID + "&loginpwd=123456&devicetype=7&aliasname=" + encodeURI(nickName);
          var userinfo_sign = getSign(userinfo_params + msmp_appkey);
          var userinfo_url = "/msmp/nscreen/modifynscreenuserinfo.action?userid=" + this.$store.state.userID + "&loginpwd=123456&devicetype=7&aliasname=" + encodeURI(nickName) + "&sign=" + userinfo_sign;
