  ...
  <div class="templ1">
    ...
    <div id="headTop">
      ...
      <div id="video">
        ...
        <video id="video1" autoplay controls x5-video-player-type="h5" @play="stopVideo" @pause="setRecord">
  ...
  export default {
    data: function () {
      return {
        ...
        liveBroadcastAddress: this.$route.query.liveBroadcastAddress,
        poster: "",
        whatDash: "show",
        toast: null,
        ...
        downloadTips: false,
        assetID: "",
        isLive: true,   //判断是否第一次进入这个页面，默认的是直播链接
        ...
        kebo: true,
        url: "",
        clickOnce: false,  //初次进来还没有点击video触发iphone开始直播，点击之后让视频reload
...
//      this.stopVideo();
    ...
    watch: {
      videoAddress(){
        ...
        window.setTimeout(function () {
          that.kebo = true;
          that.url = that.$store.state.videoAddress;
    ...
    computed: {
      videoAddress(){
//        if (this.status == 1) {
//          if (this.liveBroadcastAddress == undefined) {
//            return this.$store.state.videoAddress2;
//          } else {
//            return this.liveBroadcastAddress;
//          }
//        } else {
...
//        }
    ...
    methods: {
      ...
      scb(name, status, poster, assetID){
        this.showName = name;
        this.status = status;
        this.poster = poster;
        this.isLive = false;
      ...
      readyShare() {
        ...
        if (that.$store.state.huikanTime == undefined) {
          ...
          Share.onMenuShareAppMessage({
            ...
            imgUrl: "http://wxtv1.chsvision.com/cms/" + shareImg, // 分享图标
...
//            imgUrl: "http://122.193.16.22/cms/" + that.poster, // 分享图标
//            success: function () {
//              // 用户确认分享后执行的回调函数
//            },
//            cancel: function () {
...
//          });
          ...
          Share.onMenuShareTimeline({
            title: "我正在看:" + that.channelName + "  " + that.showName,
            link: window.location.href + "&searchType=huikan&alldate=" + alldate + "&date=" + date + "&huiName=" + that.showName + "&huiTime=" + that.$store.state.huikanTime + "&assetId=" + assetID,
            imgUrl: "http://wxtv1.chsvision.com/cms/" + shareImg,
            success: function () {
            },
            cancel: function () {
          ...
          Share.onMenuShareAppMessage({
            title: "我正在看:" + that.channelName + "  " + that.showName, // 分享标题
            desc: '快和我一起看苏州有线微信电视吧~~', // 分享描述
            link: window.location.href + "&searchType=huikan&alldate=" + alldate + "&date=" + date + "&huiName=" + that.showName + "&huiTime=" + that.$store.state.huikanTime + "&assetId=" + assetID, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
            imgUrl: "http://wxtv1.chsvision.com/cms/" + shareImg, // 分享图标
            type: '', // 分享类型,music、video或link，不填默认为link
            dataUrl: '', // 如果type是music或video，则要提供数据链接，默认为空
            success: function () {
...
//          });
      ...
      setRecord() {
        if (this.status == "2") {
          ...
          if (second != "0000") {
            var url1 = "/cms/thirdPartyPortalInterface/play.service?Symb=6900&busiType=cpg&url=&resFormat=json&Freq=35500&terminalType=mobile&assetId=" + this.assetID + "&RangeEnd=0&mediaType=3&clientId=" + this.$store.state.userID + "&Volume=0&ServiceID=101&Mod=3&RangeStart=0&PlayOffset=0";
            this.$http.get(url1).then(res => {

              var url2 = "/cms/thirdPartyPortalInterface/setResumePoint.service?clientId=" + this.$store.state.userID + "&contentId=" + this.assetID + "&busiType=cpg&terminalType=mobile&point=" + second + "&resFormat=json";
