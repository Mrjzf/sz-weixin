<template>
    ...
    <section v-for="(item1, index1) in dateList">
      ...
      <ul>
        <li class="hk" v-for="(item2, index2) in showList" v-show="item1==item2.dateinit">
          <img :src="'/cms/' + item2.poster" alt='' />
          <hgroup>
            <h1>{{ item2.contentName }}</h1>
            <h2>上次观看时间：{{ item2.playTime | formatTime }}</h2>
            <h3>观看至：{{ item2.resumePoint | formatPoint }}</h3>
          </hgroup>
        </li>
      </ul>
...
export default {
  ...
  filters: {
    ...
    formatPoint: function (value) {
      var v = value + ""
      var theTime = parseInt(v.slice(0, v.length - 3));// 秒
      var theTime1 = 0;// 分
      var theTime2 = 0;// 小时
      if(theTime > 60) {
  ...
  methods:{
    ...
    getRecord() {
      ...
      var url = "/cms/thirdPartyPortalInterfaceV2/getPlayList.service?clientId=" + this.$store.state.userID + "&busiType=cpg";
